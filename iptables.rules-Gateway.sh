...
# This script configures the firewall for a gateway (ex: between an ADSL or optic fiber connection providing an Internet connection, and the LAN).
...
start_it_up()
{
	...
	# To inspect the exchanges made by a multimedia box with following statically-assigned IP:
	...
	if [ "$enable_iptv" = "true" ] ; then
		...
		# So that the ISP content servers can join the LAN in terms of multicast:

		# Too broad authorisation:
		#${iptables} -A FORWARD -d 224.0.0.0/4 -j ACCEPT

		# Better targeted ones; must comply with the altnet clauses in /etc/igmpproxy.conf:
	...
	# Packets from the Internet interface to the LAN one must not be new unknown connections:
	...
	# Everybody from the LAN can ping me (but no one from the Internet - but the network box may answer):
