...
if [ ! -f "${shell_location}/term-utils.sh" ]; then
	previous_location=${shell_location}
	shell_location="${CEYLAN_ROOT}/src/code/scripts/shell"
	if [ ! -f "${shell_location}/term-utils.sh" ]; then
		echo "Error, unable to find the term-utils.sh helper script (searched in ${previous_location} and in ${shell_location})."
		exit 1
	fi
fi

. "${shell_location}/term-utils.sh"
...
usage="
Usage: $(basename $0) <executable to test> [<executable arguments>+]: ."
