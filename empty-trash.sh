...
usage="Usage: $(basename $0) [--simulate]: this script is to be used to empty the trash (${TRASH}) that can be filled thanks to our srm script."
...
if [ "$value" = "y" ]; then
	...
	# Deletes all files (everything which is not a directory) under ${TRASH}
	# which have not been accessed for 30 days or more.
