...
# Be sure in case this program is used remotely as root that it is run from
# a real root session (as obtained with 'ssh roo@burner', not as 
# 'ssh user@burner; su' since X-forwarding would not be effective.
xcdroast 2>/dev/null &
