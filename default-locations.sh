...
#!/bin/sh
#usage=". $(basename $0): detects the path of the most common UNIX-related tools (this script shall be sourced, not executed)."
...
#echo "Trace: begin of default-locations.sh"

# Tells whether this script has already been sourced:
default_locations_sourced=0


# Triggers termUtils.sh as well:
# Note: default-locations.sh depends on platformDetection.sh, not the contrary.

# Comparison made on strings on purpose, as may not be defined at all:
if [ ! "$platformdetection_sourced" = "0" ]; then

	platform_detect="platform-detection.sh"

	#echo "Trace: will source platform-detection.sh"
...
#echo "Trace: body of default-locations.sh"
...
# lookUpExec and findTool are to be found in platform-detection.sh
...
# Usage: findBasicShellTools [<message in case of failure>]
{
	...
	# In some cases already found by 'platform-detection.sh':
	findTool grep $1
	GREP=$returnedString
	...
	# Usually already found by 'platform-detection.sh':
	findTool uname $1
...
# Usage: findBuildTools [<message in case of failure>]
{
	...
	if  [ $is_mingw -eq 0 ]; then
		...
		# MINGW_ROOT has been set by 'platform-detection.sh':
...
#echo "Trace: auto-run of default-locations.sh"

# Auto-run on source to have basic commands:
findBasicShellTools

#echo "Trace: end of default-locations.sh"
