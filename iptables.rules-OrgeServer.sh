...
# Adapted from script 'iptables.rules-Gateway.sh' by Olivier Boudeville, on
# Sunday, August 1, 2010.

# This script configures the firewall for an Orge Server.
...
LOG_FILE=/root/lastly-orge-server-firewalled.touched

$echo "Setting Orge server firewall rules, version $version."
touch $LOG_FILE

# Only needed for older distros that do load ipchains by default, just unload
# it:
...
# These lines are here in case rules are already in place and the script is ever
# rerun on the fly.
#
# We want to remove all rules and pre-exisiting user defined chains and zero the
...
# are denied until the new rules are back in place.
#
...
# Enable response to ping in the kernel, but we will only answer if the rule at
# the bottom of the file let us:
...
# to a host take a different path than packets from that host to you), or if you
# operate a non-routing host which has several IP addresses on different
# interfaces.
#
...
# IP forwarding is turned off, as it is not a gateway:
$echo "0" > /proc/sys/net/ipv4/ip_forward
...
# Second rule is to let packets through which belong to established or related
# connections and we let all traffic out, as we trust ourself:
...
# Drops directly connections coming from the Internet with unroutable (private)
# addresses:
...
## HTTP (we suppose we host a web server):
...
## FTP (we suppose we host a FTP server):
...
# For Erlang EPMD daemon running on the non-standard, Orge-specific port:
${iptables} -A INPUT -p tcp --dport 4269 -m state --state NEW -j ACCEPT
...
## LOOPBACK
# Allow unlimited traffic on the loopback interface, e.g. needed for KDE, Gnome,
# etc.:
...
#
# (should be disabled if connected to the Internet, as far too verbose in system
# logs, might hide other important events)
#
