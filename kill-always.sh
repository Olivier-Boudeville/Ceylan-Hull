...
usage="$0 <process name to kill> [--auto-kill]: will kill, as long as this script is kept running, any process matching the specified name"
...
if [ -z "$TARGET" ]; then
	echo "Error, no target process specified.
${usage}." 1>&2
	exit 1
fi
...
echo  "    Will kill, from now on, any process matching $TARGET..."
